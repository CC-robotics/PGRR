...
Gates 0--5 are accepted on the pinned `arena_humble_docker` Gazebo profile: the synchronized classical baseline, temporary-goal recovery chain, privileged expert, observable offline/online BC pipeline, and two train-only DAgger aggregations are operational. After adding emergency-backup hysteresis, DAgger-1 reached the goal in 2/2 repeats of `crossing_flow_high_validation_s02220`; DAgger-2 reached 1/2 and is currently a negative ablation. DAgger-1 is the candidate model, but those two successes selected only CONTINUE and therefore prove non-degradation rather than active recovery benefit. Current work must obtain paired multi-scenario evidence with non-CONTINUE learned actions before Gate 6/PPO or manuscript claims. All pre-synchronization and pre-mask-alignment learning artifacts are diagnostic only.
