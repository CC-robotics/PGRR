...
- Added reset-validity classification for a goal that never becomes active. The complete quality suite now passes 162 tests and the three-package ROS overlay builds.
...
Freeze the selected 1.5 s TTC trigger and expand paired Base/Heuristic/Oracle validation seeds without further threshold tuning. Re-run high-density validation to check that the trigger improvement transfers beyond one seed, then identify a doorway variant with a finite blockage interval. Gate 3 passes online execution and trigger-selection validation but remains statistically unaccepted. Keep Gate 2 failed and do not begin neural-policy training yet.
