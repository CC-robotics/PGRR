...
Freeze the selected 1.5 s TTC trigger and use only synchronized episodes for new comparisons. Repeat the sync3 Base/Heuristic/Oracle protocol on additional validation seeds before generating large expert datasets; do not tune on high-validation seed 2220. The current single synchronized seed supports sequence-level feasibility only: Base collides, Heuristic converts collision to timeout, and Oracle reaches the goal. Gate 3 remains statistically unaccepted, Gate 2 remains failed, and neural-policy training must wait for synchronized multi-seed evidence.
