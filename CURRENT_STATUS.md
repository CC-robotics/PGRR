...
- Added reset-validity classification for a goal that never becomes active. The complete quality suite now passes 154 tests and the three-package ROS overlay builds.
...
Run sequential, paired Base/Oracle validation on additional crossing-flow train seeds and identify a doorway variant with a finite blockage interval. For cyclic doorway flow, add an explicit lateral/REPLAN phase rather than extending YIELD. Gate 3 has one successful recoverable execution but is not accepted across scenarios. Keep Gate 2 failed and do not begin neural-policy training yet.
