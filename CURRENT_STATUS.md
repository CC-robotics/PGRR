...
- Added reset-validity classification for a goal that never becomes active. The complete quality suite now passes 159 tests and the three-package ROS overlay builds.
...
Expand paired Base/Heuristic/Oracle validation seeds before changing expert weights. Use the fixed validation split to assess an explicit unnecessary-intervention penalty and retain it only if it reduces interventions without harming recovery success. Identify a doorway variant with a finite blockage interval and do not tune further on cyclic doorway seed 1100. Gate 3 passes online execution, has one favorable expert case and one high-density counterexample, and remains statistically unaccepted. Keep Gate 2 failed and do not begin neural-policy training yet.
