...
Run the high-density crossing-flow train pair, then consolidate the low/medium/high Oracle evidence without tuning on those outcomes. Identify a doorway variant with a finite blockage interval. Do not tune further on cyclic doorway seed 1100; retain it as an unrecoverable/option-segmentation diagnostic. Gate 3 now has two successful executions and one favorable failure-to-success pair, but remains pending density/multi-seed validation. Keep Gate 2 failed and do not begin neural-policy training yet.
