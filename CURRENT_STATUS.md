...
Run the heuristic on the same medium/high crossing-flow train manifests to determine whether the Oracle advantage is specific to the privileged planning expert, then expand paired validation seeds without tuning. Identify a doorway variant with a finite blockage interval. Do not tune further on cyclic doorway seed 1100. Gate 3 passes online execution and density prevalidation, but its comparative acceptance remains pending paired heuristic and multi-seed evidence. Keep Gate 2 failed and do not begin neural-policy training yet.
