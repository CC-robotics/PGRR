...
Run sequential, paired Base/Oracle validation on additional crossing-flow train seeds and identify a doorway variant with a finite blockage interval. Do not tune further on cyclic doorway seed 1100; retain it as an unrecoverable/option-segmentation diagnostic. Gate 3 has one successful recoverable execution but is not accepted across scenarios. Keep Gate 2 failed and do not begin neural-policy training yet.
