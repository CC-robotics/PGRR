#!/usr/bin/env python3
"""Safely synchronize the six preflight-pinned mask-trace runtime sources."""
...
ALLOWED_PATHS = {
    "packages/ramp_core/ramp_core/action_mask.py": 0o644,
    "packages/ramp_core/ramp_core/planning/online.py": 0o644,
    "packages/ramp_core/ramp_core/recovery/mask_trace.py": 0o644,
...
def sync(
    ...
    if not isinstance(expected, dict) or set(expected) != set(ALLOWED_PATHS):
        raise ValueError("manifest source set does not match the six-file allowlist")
...
def main() -> None:
    ...
    result = sync(args.manifest, args.source_root, args.runtime_root, args.backup_root)
