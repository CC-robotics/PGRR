...
为避免事后挑选“好看案例”，运行对比固定选择 moderate-v6 的 doorway\_bottleneck / medium / replicate 0，不依赖两个方法的终局。素材生成器从同一 Parquet 解析 Base 与 PGRR 的 episode ID，逐个校验 JSONL 的 SHA256，并导出平面轨迹、原目标距离、失败分数和恢复状态事件线。

\ifReportResultsAvailable
  \SafeFigure{generated/result_matched_run_evidence.pdf}{同一 pair 的 Base--PGRR 遥测对比。空间轨迹和时间线来自 SHA256 校验的真实 raw JSONL，并通过与 results.parquet 同目录的审计 sidecar 注入。该图是 telemetry reconstruction，不是 camera screenshot；其终局不能代替总体 outcome 分解。}
\else
  \EvidenceSlot{待生成：moderate-v6 doorway\_bottleneck / medium / r00 的 matched Base--PGRR raw/Parquet 轨迹与事件时间线。}
\fi
