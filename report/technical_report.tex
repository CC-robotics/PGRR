...
本报告把终止结果、安全、效率、稳定性、恢复行为、排除项和失败案例分开呈现。碰撞减少不会被重新解释为成功；超时和规划失败不会被隐藏。只有 stage-aware 构建器通过路径、split、pair 和完整性检查后，结果页才会出现数值。
...
本项目研究的问题是：\emph{能否在不破坏正常场景稳定性的前提下，只在传统规划器即将失败时，利用规划专家生成的示范和分布聚合学习恢复决策？}
...
\ifReportResultsAvailable
  ...
  \ReportBaseGoalRate{} 和 \ReportPGRRGoalRate{}。统计显著性必须另由锁定的配对统计 JSON 支持，不能由这三个描述量推断。
...
结果构建器执行以下硬检查：
\begin{enumerate}
  \item pending 模式不得接收任何 results 路径；
  \item historical、pilot、calibration、smoke 与运行中的 validation 路径在打开前被拒绝；
  \item validation 只能读取显式完成快照，test 只能读取 \texttt{outputs/moderate/final/results.parquet}；
  \item split 必须与报告阶段唯一一致；
  \item 每个 pair 必须恰好包含五种方法且场景元数据一致；
  \item 终止类型必须属于预声明集合；
  \item 报告 JSON、TeX 宏和图表共享同一结果 SHA256。
\end{enumerate}
