...
\input{generated/report_stage.tex}
...
\newcommand{\EvidenceSlot}[1]{%
  ...
  \fcolorbox{PGRRBlue}{PGRRLight}{%
    ...
    \centering\bfseries
    #1\par
    缺少批准素材时保持空槽并拒绝 result-stage 发布；不得以示意图、旧结果或推测曲线代替。
    \end{minipage}}
  \end{center}}
...
\title{\bfseries PGRR: Planning-Guided Failure-Triggered Recovery and Rejoin\\
for Dynamic Social Navigation\\
\large 系统、算法、实验与复现技术报告}
\author{Charles Chen}
\date{2026 年 8 月}
...
\begin{titlepage}
  ...
  PGRR: Planning-Guided Failure-Triggered\\Recovery and Rejoin for\\Dynamic Social Navigation\par}
  \vspace{7mm}
...
作为不可覆盖的历史审计记录，旧版 v1 已完成 64/64 个 logical episodes：Base 与 PGRR 各覆盖 24 个 family--density 条件，Standard 与 Heuristic 各覆盖 8 个 high-density 条件。PGRR 碰撞为 0/24、Base 为 19/24；但 PGRR 超时为 16/24、Base 为 0/24。PGRR 到达 8/24、Base 到达 5/24，且成功率的校正比较不显著。这只说明旧版存在明确的安全--完成权衡，不是 moderate-v6 validation/test 结果，也不得进入 v6 的表格、图或性能结论。
...
\clearpage
\section{贡献、边界与命名}
公开名称固定为 \textbf{PGRR: Planning-Guided Failure-Triggered Recovery and Rejoin for Dynamic Social Navigation}。历史 \texttt{ramp\_*}/\texttt{RAMP\_*} 仅为兼容接口，不是公开方法名。当前可验证的贡献边界是：
...
PPO、学习型失败检测器、第二规划器、Flatland、硬件实验和形式化安全不是当前版本的完成声明。报告不会使用“首次结合”“保证安全”或“解决社会导航”等不可验证表述。
...
\section{真实运行环境与边界}
在线运行环境是 Ubuntu 22.04、ROS2 Humble 和固定版本的 Arena/RosNav Gazebo profile；机器人为 Jackal，导航栈采用 Nav2 与 DWB，本体观测来自平面 LiDAR、里程计、TF、路径和 planner 状态。运行节点统一支持 \texttt{use\_sim\_time}。为避免 ROS ABI 与 Python 包污染，在线容器/venv 与离线 \texttt{ramp-offline} Conda 环境严格分离。

Gazebo 中的动态行人由可见的确定性圆柱代理按 seeded route 运动。该 fallback 能产生可复现的 LiDAR 交互，但不是 HuNav 社会力、真实人体行为或硬件验证。软件渲染和仿真调度也可能影响 wall-clock，定量指标以仿真时间与终止证据为准。

\clearpage
\section{Arena/Gazebo 运行证据}
\SafeFigure[0.78\textwidth]{runtime_gazebo_doorway_bottleneck_medium.png}{真实 Arena Gazebo GUI 冻结场景演示：Ubuntu 22.04 / ROS2 Humble 下，Jackal、Nav2 DWB、平面 LiDAR 可见动态代理在 doorway\_bottleneck、medium、validation 场景运行。截图像素、窗口、日志、commit 与 SHA256 有元数据。它来自审计过的 v5 validation 演示，不是锁定 v6 统计回合的 camera frame。}

该截图只证明对应运行时、机器人、行人和场景能够实际启动与完成，不承担定量效果证据。定量结论只能来自完整 manifest 的 episode-level Parquet。后文轨迹图由真实 JSONL 遥测重建，必须明确标为 telemetry reconstruction，绝不称为 simulator camera screenshot。

\clearpage
\section{Matched Base--PGRR 真实运行证据}
Validation 快照采用预注册的 doorway\_bottleneck / medium / replicate 0，且不依赖两个方法的终局。锁定 test 则使用已冻结的确定性素材规则：先要求 PGRR 实际触发恢复、场景同时包含配置静态几何与行人路线，再按公开顺序选择一个描述性 pair。素材生成器从同一 Parquet 解析 Base 与 PGRR 的 episode ID，逐个校验 raw、metadata、outcome、scenario 与 results 的 SHA256，并导出平面轨迹和 PGRR 恢复时间线。固定文件名与哈希写入 \texttt{matched\_base\_pgrr\_evidence.json}；任何缺失或不一致均阻止 final 构建。

\ifReportResultsAvailable
  \ifReportFixedTestMediaAvailable
    \SafeFigure{generated/result_matched_trajectory.pdf}{锁定 test 中同一 pair 的 Base--PGRR 平面轨迹。场景几何、行人路线与运动轨迹来自 SHA256 绑定的真实配置和 raw JSONL。该图是 telemetry reconstruction，不是 camera screenshot；单个 pair 的终局不能代替总体 outcome 分解。}
    \SafeFigure{generated/result_matched_recovery_timeline.pdf}{同一 test pair 的 PGRR 恢复时间线，包含记录的目标距离、failure score、恢复状态与触发事件。它是 telemetry reconstruction，不是 camera screenshot。}
  \else
    \SafeFigure{generated/result_matched_run_evidence.pdf}{Validation 同一 pair 的 Base--PGRR 遥测对比。空间轨迹和时间线来自 SHA256 校验的真实 raw JSONL，并通过审计 sidecar 注入。该图是 telemetry reconstruction，不是 camera screenshot；其终局不能代替总体 outcome 分解。}
  \fi
\else
  \EvidenceSlot{待生成：固定文件名 \texttt{moderate\_matched\_base\_pgrr\_trajectory.pdf} 的同一 test pair Base--PGRR 真实遥测轨迹。}
  \EvidenceSlot{待生成：固定文件名 \texttt{moderate\_pgrr\_recovery\_timeline.pdf} 的同一 test pair PGRR 恢复事件时间线。两者均是 telemetry reconstruction，不是 camera screenshot。}
\fi
...
权威基准为 moderate-v6。最终 test 协议预注册 120 个条件，每个条件由 DWB、Standard、Heuristic、Uniform BC 和 PGRR 五种方法共享，总计 600 个 logical method--episodes。validation 快照则必须有独立标签，不得包装成 test 结论。

moderate-v5 的完整 validation Base 为 57/72 到达（79.17\%），超过预注册的 75\% 上限，因此校准状态为 rejected；其 test 从未打开。v6 保留 v5 静态几何，仅根据 v5 validation 中 Crossing Flow 9/9 到达且交互距离过大的审计证据，预注册较慢的 crossing 时序与全新 seed block。该拒绝历史必须保留，但 v5 数字不能进入 v6 final 结果表。
...
\section{配对统计与基础设施排除}
预注册的到达、碰撞和超时二元端点使用完全相同 pair key 的精确 McNemar 检验；规划失败只做描述性报告。连续配对差异使用双侧 Wilcoxon signed-rank。均值配对效应使用固定 seed 的 10,000 次 percentile bootstrap 95\% 区间，预注册比较族统一进行 Holm 校正，并同时报告效应量和区间。
...
\ifReportResultsAvailable
  ...
  \input{generated/result_table.tex}
  \SafeFigure{generated/result_outcomes.pdf}{五种方法的完整终止结果。碰撞、超时和规划失败均未折叠。}
  DWB 与 PGRR 的 \texttt{PLANNER\_FAILURE} 描述性率分别为
  \ReportBasePlannerFailureRate{} 和 \ReportPGRRPlannerFailureRate{}，PGRR--DWB
  边际率差为 \ReportPlannerFailureDifference{}。该类别完整保留，但不是预注册推断端点，
  因此不补做事后显著性检验。
\else
  \PendingResults
\fi
...
\ifReportResultsAvailable
  ...
  \input{generated/result_paired_statistics.tex}
  ...
  \ifReportJointSuccessEfficiencyAvailable
    在 DWB 与 PGRR 都到达目标的 \ReportJointSuccessPairCount{} 个 pair 中，PGRR--DWB
    episode 时长配对差为 \ReportJointSuccessDurationDifference{}
    （$p_{\mathrm{Holm}}\ReportJointSuccessDurationHolmP$），路径长度配对差为
    \ReportJointSuccessPathDifference{}（$p_{\mathrm{Holm}}\ReportJointSuccessPathHolmP$）。
    这两项直接来自 pairwise JSON，条件于共同成功，不能删除或替代失败终局。
  \else
    没有有限的 DWB--PGRR 共同到达 pair；时长和路径效率不可估计，终止结果仍为主证据。
  \fi
\else
  \PendingResults
\fi
...
\clearpage
\section{局限性与有效性威胁}
当前系统依赖二维 LiDAR、已知静态地图、离散恢复动作和仿真行人模型。特权专家依赖仿真真值；安全监督器没有形式化保证；同一地图和场景模板上的 held-out seed 不等价于跨地图泛化；Gazebo 截图不能代替实机实验。PPO、学习型失败检测器、第二规划器、Flatland、硬件与形式化安全均未完成。
...
结果构建器执行以下硬检查：
\begin{enumerate}
  \item pending 模式不得接收任何 results、statistics 或 matched-evidence 路径；
  \item historical、pilot、calibration、smoke、rejected v5，以及
  \nolinkurl{outputs/moderate/v6_validation} 和所有
  \nolinkurl{outputs/moderate/v6_validation_*} 活跃/独立验证路径在打开前被拒绝；
  \item validation 只能读取显式完成快照；test 只能读取
  \nolinkurl{outputs/moderate/final/} 下同目录、同一结果哈希绑定的
  results Parquet、pairwise statistics 与 matched evidence 三个产物；
  \item scenario/pair ID 必须属于 moderate-v6，防止把 rejected v5 复制到批准目录；
  ...
  \item matched sidecar 必须绑定 results SHA、固定 selector、Base/PGRR episode ID、终局和 raw SHA；
  \item 报告 JSON、TeX 宏和图表同时保存结果、统计与 matched evidence 的 SHA256。
\end{enumerate}
