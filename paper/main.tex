...
Table~\ref{tab:cross-family-pilot} and Fig.~\ref{fig:cross-family-pilot} add four high-density, single-seed checks from distinct scenario families. In the current same-commit temporary-blockage pair, DWB collided at 38.00 s and the hierarchy collided at 40.03 s after 65 non-CONTINUE samples. A historical 80.92 s success at the same scenario seed is therefore not treated as repeatable evidence. In the current same-commit group-blocking pair, DWB collided at 46.02 s while the hierarchy reached the physical goal at 128.94 s with 0.211 m terminal error and 1.019 m minimum human distance. The learned policy selected temporary-subgoal actions 3 and 11 alongside separately logged STOP, BACKUP, TURN, and FORWARD safety modes. In the frozen overtaking pair, DWB collided at 82.02 s while the hierarchy physically reached the goal at 178.39 s with 0.224 m terminal error and 1.156 m minimum human distance. Both overtaking sensor gates passed every near-human frame, but completion had only 1.61 s timeout margin and required 1,133 recovery samples. Finally, in blind corner, DWB intersected declared static geometry after 49.48 s. The selected hierarchy remained collision-free and made 13.47 m progress, but timed out 8.16 m from the physical goal. This is collision avoidance, not recovery success.
...
A separate train-only attempt classified the closest LiDAR endpoint as static when it lay near known shelf geometry. It improved blind-corner progress, and removing duplicated radial braking even reached the train goal. Cross-scenario checks nevertheless rejected the idea: the no-radial-braking variant drifted into a doorway during skid-steer rotation, while the restored-braking classifier treated a pedestrian beside that doorway as static and collided at 0.705 m centre distance. We restored uniform collision-latched margins. This illustrates why a local performance gain cannot override a retained safety counterexample.
