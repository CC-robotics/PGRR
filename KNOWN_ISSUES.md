...
The selected hierarchy applied the 0.85 m collision-latched human-safety margin to every nearest LiDAR return. In both held-out and train blind-corner scenarios, the closest return belonged to declared shelf geometry; the robot remained collision-free but spent roughly 70 s alternating STOP, BACKUP, TURN, and short FORWARD pulses near the wall. The train run eventually cleared the corner but timed out 5.09 m from goal. An observable classifier projects only the nearest scan endpoint into the known shelf boxes; its first train candidate reduced terminal distance to 1.01 m but still timed out. A second candidate removed braking inflation from the radial static footprint threshold, reached the train goal, and improved held-out blind-corner progress, but then physically hit a temporary-blockage door frame while turning. That unsafe refinement is rejected. The full braking-distance term is restored; unmatched returns continue to use 0.85/0.65 m dynamic margins.
