...
The selected hierarchy applied the 0.85 m collision-latched human-safety margin to every nearest LiDAR return. In both held-out and train blind-corner scenarios, the closest return belonged to declared shelf geometry; the robot remained collision-free but spent roughly 70 s alternating STOP, BACKUP, TURN, and short FORWARD pulses near the wall. The train run eventually cleared the corner but timed out 5.09 m from goal. A candidate observable classifier projects only the nearest scan endpoint into the known shelf boxes. Matching static returns use a 0.50 m stop margin and 0.36 m action clearance, both above the 0.36 m robot radius; unmatched returns retain 0.85/0.65 m dynamic margins. Online train validation is required before selection.
