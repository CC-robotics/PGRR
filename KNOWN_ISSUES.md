...
The selected hierarchy applied the 0.85 m collision-latched human-safety margin to every nearest LiDAR return. In both held-out and train blind-corner scenarios, the closest return belonged to declared shelf geometry; the robot remained collision-free but spent roughly 70 s alternating STOP, BACKUP, TURN, and short FORWARD pulses near the wall. The train run eventually cleared the corner but timed out 5.09 m from goal. An observable classifier now projects only the nearest scan endpoint into the known shelf boxes. Its first train candidate reduced terminal distance to 1.01 m but still timed out. Trace review found that radial footprint clearance also included forward braking distance even for tangential travel beside a known wall. The refined candidate keeps fixed 0.50 m static radial clearance while the independent forward-sector test retains the full braking model; unmatched returns retain 0.85/0.65 m dynamic margins. A fresh train replay is required before selection.
